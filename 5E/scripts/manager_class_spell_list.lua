...
function helperUpdateClassSpellListItems(vNode, rRecord)
	...
	for _,v in ipairs(DB.getChildList(vNode, "spells")) do
		local sKey = StringManager.simplify(DB.getValue(v, "name", ""));
...
function rebuildClassSpellLists()
	...
	for nodeSpell,vSpellRecord in pairs(_tSpellRecords) do
		local sSpellKey = StringManager.simplify(vSpellRecord.sDisplayName);
		...
		for _,sClassName in pairs(vSpellRecord.tSources) do
			local sClassKey = StringManager.simplify(sClassName);
	...
	for nodeClassSpellList, vClassSpellListRecord in pairs(_tClassSpellListRecords) do
		local sClassKey = StringManager.simplify(vClassSpellListRecord.sDisplayName);
		if not tTempClasses[sClassKey] then
