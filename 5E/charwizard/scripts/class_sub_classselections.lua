...
function rebuildListDisplay()
	parentcontrol.window.filter_class.clear();
	for k,v in pairs(_tModules) do
		parentcontrol.window.filter_class.add(k);
	end
	parentcontrol.window.filter_class.add("");
end
...
function addDisplayListItem(v)
	local wClass = list.createWindow(v.vNode);
	wClass.name.setTooltipText(v.sModule);
