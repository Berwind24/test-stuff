...
function onModuleUpdated(sModule)
	...
	if tInfo then
		ModuleManager.setModuleInfo(sModule, tInfo);
		if not tInfo.loaded and ModuleManager.isModuleLoaded(sModule) then
			ModuleManager.setLoadedModuleInfo(sModule, nil);
		end
...
function onLibraryModuleActivate(sModule)
	...
	wLibrary.pagelist.setDatabaseNode(nodeSource);
	wLibrary.pagelist.setVisible(true);
end
function onLibraryAuxBookActivate(sModule)
	...
	wLibrary.pagelist.setDatabaseNode(DB.findNode(tInfo.sourcepath));
	wLibrary.pagelist.setVisible(true);
end
