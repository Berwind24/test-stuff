...
function handleStandardCombatAddPCFields(tCustom)
	...
	DB.setValue(tCustom.nodeCT, "link", "windowreference", tCustom.sClass, DB.getPath(tCustom.nodeRecord));
...
function onBattleButtonAdd(wBattle)
	...
	for _,vNPC in pairs(wBattle.npcs.getWindows()) do
		for _,vMapLink in pairs(vNPC.maplinks.getWindows()) do
			vMapLink.deleteLink();
		end
	end
	if CombatRecordManager.hasRecordTypeCallback("battle") then
		CombatRecordManager.onRecordTypeEvent("battle", { sRecordType = "battle", nodeRecord = wBattle.getDatabaseNode() });
	end
end
