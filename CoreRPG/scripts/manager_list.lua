...
function helperSetupSource(w, rInfo)
	...
	if ((sModule or "") ~= "") then
		if w.label_source then
			w.label_source.setValue(Interface.getString("ref_view_source_module"));
		end
		if w.source_module then
			local tModuleInfo = Module.getModuleInfo(sModule);
			if tModuleInfo then
				w.source_module.setValue(tModuleInfo.displayname);
				w.source_module.setVisible(true);
			end
		end
	else
		if w.label_source then
			w.label_source.setValue(Interface.getString("ref_view_source_category"));
		end
		if w.source_category then
			...
			for _,v in ipairs(tCategories) do
				w.source_category.add(v.sValue, v.sText);
			end
			w.source_category.setListIndex(1);
			w.source_category.setComboBoxVisible(true);
		end
	end
end
