...
function performInit(wExport)
	...
	for _,rExport in ipairs(tExports) do
		local wRecordType = wExport.recordtypes.createWindow(DB.getPath("export.recordtypes", rExport.name));
		wRecordType.setData(rExport);

		local tViews = LibraryData.getRecordViews(rExport.name);
		if tViews then
			for kView, tView in pairs(tViews) do
				local wRecordView = wExport.recordviews.createWindow(DB.getPath("export.recordviews", rExport.name, kView));
				local tRecordView = {};
