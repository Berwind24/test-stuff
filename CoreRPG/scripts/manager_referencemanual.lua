...
function init(sModule, sPath)
	...
	if not _tManualIndexPath[sModule] then
		...
		if nChapterEnd then
			...
			sManualPath = DB.getPath(ReferenceManualManager.MANUAL_DEFAULT_INDEX, ReferenceManualManager.MANUAL_DEFAULT_CHAPTER_LIST_NAME);
		end
...
function getOrderedRecords(tDefaultRecords)
	...
	for k,v in pairs(tDefaultRecords) do
		table.insert(tSorter, { nIndex = k, nOrder = DB.getValue(v, "order", 0), sPath = DB.getPath(v) });
	end
...
function updateOrderValues(cList)
	...
	for _,wChild in ipairs(cList.getWindows()) do
		...
		local sName = DB.getName(wChild.getDatabaseNode());
...
function onIndexAdd(w)
	...
	if wPage then
		...
		local sModule = DB.getModule(nodeList);
		if (sModule or "") ~= "" then
...
function onCampaignKeywordGen()
	for _,nodeChapter in ipairs(DB.getChildList(DB.getPath(ReferenceManualManager.MANUAL_DEFAULT_INDEX, ReferenceManualManager.MANUAL_DEFAULT_CHAPTER_LIST_NAME))) do
		for _,nodeSubchapter in ipairs(DB.getChildList(nodeChapter, ReferenceManualManager.MANUAL_DEFAULT_SUBCHAPTER_LIST_NAME)) do
			for _,nodePage in ipairs(DB.getChildList(nodeSubchapter, ReferenceManualManager.MANUAL_DEFAULT_PAGE_LIST_NAME)) do
...
function migratePageLegacyTextToBlock(wPage)
	...
	if bStatic then
		...
		DB.setStatic(wPage.blocks.getDatabaseNode(), true);
		for _,wChild in ipairs(wPage.blocks.getWindows()) do
			local nodeChild = wChild.getDatabaseNode();
			DB.setStatic(nodeChild, true);
			DB.setStatic(DB.createChild(nodeChild, "order"), true);
		end
	end
